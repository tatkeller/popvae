...
def plot_images(x,r,c,filename="problem1.png"):
    x = x.detach().numpy()
    fig = plt.figure()
    for i in range(r*c):
        image_to_plot = np.reshape(x[i],(28,28))
        sp = fig.add_subplot(r,c,i+1)
        sp.set_axis_off()
        sp.imshow(image_to_plot,cmap='gray')
...
def sample_gaussian(m, v):
    ...
    batch,dim = m.shape
    sn = torch.normal(mean=torch.zeros(batch,dim),std=torch.ones(batch,dim))
    z = m + sn * torch.sqrt(v)
    return z
...
def log_normal(x, m, v):
    ...
    """
    elem_wise_log_prob = -0.5 * (torch.log(2 * np.pi * v) + torch.square(x-m) / v)
    log_prob = elem_wise_log_prob.sum(-1)
    return log_prob
...
def log_normal_mixture(z, m, v):
    ...
    """
    batch,mix,dim = m.shape 
    mix_wise_log_prob = torch.zeros(batch,mix)
    for i in range(mix):
        mix_wise_log_prob[:,i] = log_normal(z,m[:,i,:],v[:,i,:])
    # take mean over mix
    log_prob = log_mean_exp(mix_wise_log_prob,dim=1)
    return log_prob
...
def get_mnist_data(device, use_test_subset=True):
    preprocess = transforms.ToTensor()
    train_loader = torch.utils.data.DataLoader(
        datasets.MNIST('data', train=True, download=False, transform=preprocess),
        batch_size=100,
        shuffle=True)
    test_loader = torch.utils.data.DataLoader(
        datasets.MNIST('data', train=False, download=False, transform=preprocess),
        batch_size=100,
        shuffle=True)
